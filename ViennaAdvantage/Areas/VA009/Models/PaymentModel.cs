...
namespace VA009.Models
{
    public class PaymentModel
    {
        ...
        public List<PaymentData> GetPaymentData(int pageNo, int pageSize, Ctx ctx, string whereQry, string SearchText, string WhrDueDate, string TransType, string FromDate, string ToDate)
        {
            ...
            DataSet ds = VIS.DBase.DB.ExecuteDatasetPaging(sql.ToString(), pageNo, pageSize);
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                //for paging 
                if (pageNo == 1)
                    countRecords = Util.GetValueOfInt(DB.ExecuteScalar("SELECT COUNT(*) FROM ( " + sql.ToString() + " )"));
        ...
        public List<PaymentData> GetChquePopUpdata(Ctx ctx, string InvPayids, int bank_id, int acctno, string chkno, string OrderPayids)
        {
            ...
            sql.Append(@" GROUP BY pm.VA009_PaymentMode, pm.VA009_PaymentMethod_ID, cb.c_Bpartner_id,  cb.name, inv.DocumentNo, cs.C_Order_ID,
                          ...
                          cs.AD_Client_ID, cc.ISO_CODE ORDER BY C_Bpartner");           // Order By Business Partner name not on ID
            ...
            try
            {
                ...
                if (PaymentData.Length > 0)
                {
                    ...
                    int no = Util.GetValueOfInt(DB.ExecuteScalar(@"SELECT COUNT(*) FROM C_Cash WHERE IsActive = 'Y' AND DocStatus NOT IN ('CO' , 'CL', 'VO')  
                                 AND to_date(DateAcct, 'dd/mm/yyyy') != to_date(SYSDATE, 'dd/mm/yyyy') AND C_CashBook_ID = " + C_CashBook_ID, null, null));
                    if (no > 0)
                    {
                    ...
                    else
                    {
                        C_Cash_ID = Util.GetValueOfInt(DB.ExecuteScalar(@"SELECT C_Cash_ID FROM C_Cash WHERE IsActive='Y' AND DocStatus='DR' 
                                AND to_date(SYSDATE, 'dd/mm/yyyy') = to_date(SYSDATE, 'dd/mm/yyyy') AND C_Cashbook_ID=" + C_CashBook_ID + "", null, null));
                    }
                    ...
                    else
                    {
                        ...
                        _Cash.SetAD_Org_ID(ct.GetAD_Org_ID());
        ...
        public string CrateBatchPayment(Ctx ct, GeneratePaymt[] PaymentData)
        {
            ...
            return Msg.GetMsg(ct, "VA009_SavedSuccessfully");
        }
        ...
        {
            MBankAccount _bnkAcct = new MBankAccount(ctx, bankaccount, null);
            return _bnkAcct.GetC_Currency_ID();
        }
        //end
        ...
        public List<Dictionary<string, object>> LoadBankAccount(int c_Bank_ID, string orgs, Ctx ct)
        {
            ...
            if (orgs.Length > 0)
            {
                if (c_Bank_ID == 0)
                    ...
                    qry.Append(" WHERE acct.IsActive='Y' AND acct.AD_Client_ID =" + ct.GetAD_Client_ID() + " AND acct.AD_Org_ID IN (" + orgs + ")");
            }
            else if (c_Bank_ID == 0)
            {
        ...
        /// <returns>String, Message</returns>
        public string CreatePaymentsMannualy(Ctx ct, string InvoiceSchdIDS, string OrderSchdIDS, int BankID, int BankAccountID, int PaymentMethodID, string DateAcct, string CurrencyType, string DateTrx)
        {
            ...
            try
            {
                    ...
                    else
                    {
                        for (int i = 0; i < invoiceIds.Length; i++)
                        {
                            ...
                            if (PartnerID.Contains(_invoice.GetC_BPartner_ID()))
                            {
                                ...
                                MPaymentAllocate M_Allocate = new MPaymentAllocate(ct, 0, trx);
                                M_Allocate.SetAD_Org_ID(_invoice.GetAD_Org_ID());
                                ...
                                if (_doctype.GetDocBaseType() == "APC" || _doctype.GetDocBaseType() == "API")
                                {
                                    if (_payschedule.GetDueAmt() < 0)
                                        M_Allocate.SetAmount(-1 * (_payschedule.GetDueAmt()));
                                    else
                                        M_Allocate.SetAmount(_payschedule.GetDueAmt());

                                    if (_payschedule.GetDiscountAmt() < 0)
                                        M_Allocate.SetDiscountAmt(-1 * (_payschedule.GetDiscountAmt()));
                                    else
                                        M_Allocate.SetDiscountAmt(_payschedule.GetDiscountAmt());
                            ...
                            else
                            {
                                ...
                                if (Found == true)
                                {
                                    ...
                                    _pay.SetAD_Org_ID(_invoice.GetAD_Org_ID());
                                    _pay.SetC_DocType_ID(_doctype_ID);
                                    _pay.SetDateAcct(Util.GetValueOfDateTime(DateAcct));
                                    // to set date trx
                                    _pay.SetDateTrx(Util.GetValueOfDateTime(DateTrx));
                                ...
                                else
                                {
                                    ...
                                    if (_doctype.GetDocBaseType() == "APC" || _doctype.GetDocBaseType() == "API")
                                    {
                                        ...
                                        {
                                            if (_payschedule.GetDueAmt() > 0)
                                                _pay.SetPayAmt(-1 * (_payschedule.GetDueAmt()));// -1 Received amount can't be nagative
                                            else
                                                _pay.SetPayAmt(_payschedule.GetDueAmt());
                                        }

                                        if (_payschedule.GetDiscountAmt() < 0)
                                            _pay.SetDiscountAmt(-1 * (_payschedule.GetDiscountAmt()));
                                        else
                                        {
                                            _pay.SetDiscountAmt(_payschedule.GetDiscountAmt());
                                        }
                ...
                if (OrderIds.Length > 0)
                {
                    for (int i = 0; i < OrderIds.Length; i++)
                    {
                        ...
                        _pay.SetAD_Org_ID(Util.GetValueOfInt(orderPaySchedule.GetAD_Org_ID()));
