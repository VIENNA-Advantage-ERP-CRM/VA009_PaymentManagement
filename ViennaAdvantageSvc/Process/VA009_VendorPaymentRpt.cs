...
{
    public class VA009_VendorPaymentRpt : SvrProcess
    {
        #region Variables
        ...
        protected override string DoIt()
        {
            ...
            _Sql.Append(@"CREATE OR REPLACE FORCE  VIEW  VA009_VENDORPAYMENTRPT_V
                                AS
                                SELECT C_PROJECTREF_ID,datetrx,T.C_CAMPAIGN_ID,C_CHARGE_ID,C_Bankaccount_Id,T.ad_client_id,T.ad_org_id,View_ID,documentno, 
                                  ...
                                  SELECT P.C_PROJECTREF_ID,
                                    ...
                                    ROUND(P.Payamt,4) Payamt,
                                    ROUND(P.Writeoffamt,4) Writeoffamt,
                                    ROUND(P.Discountamt,4) Discountamt,
                                    ROUND(P.Overunderamt,4)Overunderamt,
                                    CASE P.Isallocated
                                      WHEN 'Y'
                                      THEN 'YES'
                                      WHEN 'N'
            ...
            _Sql.Append(@" WHERE P.Docstatus                IN ('CL','CO')
                                  ...
                                  SELECT 0 AS C_PROJECTREF_ID,
                                    ...
                                    CASE 
                                    WHEN CL.VSS_PAYMENTTYPE= 'P'
                                    THEN 
                                    ROUND((CL.Amount * -1), 4) 
                                    WHEN ( CL.VSS_PAYMENTTYPE= 'R' AND CBT.DocBaseType='APC' ) 
                                    THEN ROUND((CL.Amount * -1), 4)  END as Amount,
                                    ROUND(CL.Writeoffamt,4) as Writeoffamt,
                                    ROUND(CL.Discountamt,4) as Discountamt,
                                    ROUND(CL.Overunderamt,4) as Overunderamt ,
                                    CASE CL.Isallocated
                                      WHEN 'Y'
                                      THEN 'YES'
                                      WHEN 'N'
                                  ...
                                  left join C_DOCTYPE CBT
                                  ON ( CBT.C_doctype_id =inv.C_doctype_id)
                                  LEFT JOIN C_Project PR
                                  ON PR.C_PROJECT_ID=C.C_PROJECT_ID
                                  LEFT JOIN C_Campaign CM
                                  ON CM.C_CAMPAIGN_ID    =C.C_CAMPAIGN_ID
                                  WHERE C.Docstatus     IN ('CL','CO')
                                   AND (
                                  CASE
                                    WHEN CBT.DOCBASETYPE  ='APC'
                                    AND CL.VSS_PAYMENTTYPE='R'
                                    THEN 'Y'
                                    WHEN CBT.DOCBASETYPE  ='ARC'
            ...
            _Sql.Append(" ORDER BY DOCUMENTNO  DESC");
            // Execution of view
            try
            {
                ...
                if (no == -1)
                {
                    DB.ExecuteQuery("DROP VIEW Va009_Vendorpaymentrpt_V");
                }
            }
            ...
            _Sql.Append(@"Create or Replace Force view Va009_VendorpaymentrptSUB_V AS 
                                         SELECT ");
            if (_C_BankAccount_ID_ > 0)
            {
            ...
            _Sql.Append(@"   ad_client_id,
                                  0   as ad_org_id,
                                  c_currency_id , 
                                  ROUND(SUM(PAYAMT),4)  PAYAMT,
                                  ROUND(SUM(WRITEOFFAMT),4) WRITEOFFAMT,
                                  ROUND(SUM(DISCOUNTAMT),4) DISCOUNTAMT,
                                  ROUND(SUM(OVERUNDERAMT),4)OVERUNDERAMT  
                                  FROM VA009_VENDORPAYMENTRPT_V
                                  ");
            ...
            try
            {
                ...
                if (no == -1)
                {
                    DB.ExecuteQuery("DROP VIEW Va009_VendorpaymentrptSUB_V");
                }
            }
            catch (Exception ex)
            {
