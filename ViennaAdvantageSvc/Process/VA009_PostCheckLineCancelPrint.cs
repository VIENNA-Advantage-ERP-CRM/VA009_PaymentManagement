 ...
 * Class Name     : VA009_PostCheckLineCancelPrint
 * Purpose        : Print Cancelled Check From Post Dated Check Line Window
...
{
    class VA009_PostCheckLineCancelPrint : SvrProcess
    {
        #region Variables
        ...
        protected override string DoIt()
        {
            ...
            try
            {
                ...
                if ((DocStatus == "RE" || DocStatus == "VO") && DocumentBaseType == "PDP" && PDCLine_ID > 0)
                {
                    ...
                    else
                    {
                        ...
                        ds.Dispose();
                        _sql.Clear();
                        _sql.Append("Update AD_Process  Set SqlQuery='" + sqlQuery + "' , ReportPath='" + path + "' Where NAme='VA009_CancelCheckPrintLineReport'");
                        count = 0;
                        count = Convert.ToInt32(DB.ExecuteQuery(_sql.ToString(), null, Get_Trx()));
                    ...
                    if (ds != null && ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        _sql.Clear();
                        _sql.Append("Update VA027_CHEQUEDETAILS set VA009_IsCancelled='Y' where isactive='Y' and VA027_CHEQUEDETAILS_ID=" + PDCLine_ID);
                        count = 0;
                        count = Convert.ToInt32(DB.ExecuteQuery(_sql.ToString(), null, Get_Trx()));
 
                        _sql.Clear();
                        _sql.Append("SELECT COUNT(*) FROM VA027_CHEQUEDETAILS WHERE VA027_POSTDATEDCHECK_ID=" + PDC_id + " AND VA009_IsCancelled='N' AND isactive='Y'");
